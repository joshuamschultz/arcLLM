...
2. **Evaluation**: `pip install arcllm`, reads types, tries `load_model()` + `.invoke()` in a script
...
|-------|------------|----------------|
| Evaluation | `pip install` + first `.invoke()` call | Works in <5 minutes |
...
|--------|-------------|-----------------|
| Time to First Call | Minutes from `pip install` to successful `.invoke()` | Manual benchmark |
